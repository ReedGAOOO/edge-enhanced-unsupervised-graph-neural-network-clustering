...
CONDITION_ALIASES = {
    "B15_PATHB_v12_hier": "B15_ECHF_s60",
...
def condition_space() -> List[Dict[str, object]]:
    # Unified branch-structure comparison on real urban graphs.
    return [
        {
        ...
        {
            # Preset-aligned G17 comparator on urban datasets.
            "condition": "G17_V5_temp15",
            "edge_variant": "V5",
            "edge_hybrid_alpha": 0.7,
            "edge_feat_temp": 1.0,
            "edge_input_prior_alpha": 0.35,
        ...
        {
            # Preset-aligned G15 (default) for heterophilic/high-heterogeneity graphs.
            "condition": "G15_ECHF_main",
            "edge_variant": "V12",
            "edge_hybrid_alpha": 0.7,
            "edge_feat_temp": 1.0,
            "edge_input_prior_alpha": 0.35,
        ...
        {
            # Preset-aligned G15 (no adaptive alpha) variant.
            "condition": "G15_ECHF_noadapt",
            "edge_variant": "V12",
            "edge_hybrid_alpha": 0.7,
            "edge_feat_temp": 1.0,
            "edge_input_prior_alpha": 0.35,
...
def build_cmd(repo: Path, dataset: str, seed: int, version: str, cond: Dict[str, object], args) -> List[str]:
    cmd = [
        sys.executable,
        "main.py",
        "--dataset",
...
def aggregate(out_dir: Path, runs: pd.DataFrame) -> None:
    ...
    by_cd = (
        ...
        .agg(
            ...
            ari_std=("ari", "std"),
            acc_mean=("acc", "mean"),
            nmi_trial_std_mean=("nmi_trial_std", "mean"),
            ari_trial_std_mean=("ari_trial_std", "mean"),
            stability_pair_nmi_mean=("stability_pair_nmi", "mean"),
            si_loss_mean=("si_loss", "mean"),
            modularity_mean=("modularity", "mean"),
            conductance_mean=("conductance_mean", "mean"),
            conductance_weighted_mean=("conductance_weighted", "mean"),
            pred_n_clusters_mean=("pred_n_clusters", "mean"),
            pred_cluster_size_cv_mean=("pred_cluster_size_cv", "mean"),
            known_label_ratio_mean=("known_label_ratio", "mean"),
    ...
    by_cond = (
        ...
        .agg(
            ...
            mean_nmi_trial_std=("nmi_trial_std_mean", "mean"),
            mean_ari_trial_std=("ari_trial_std_mean", "mean"),
            mean_stability_pair_nmi=("stability_pair_nmi_mean", "mean"),
            mean_si_loss=("si_loss_mean", "mean"),
            mean_modularity=("modularity_mean", "mean"),
            mean_conductance=("conductance_mean", "mean"),
            mean_conductance_weighted=("conductance_weighted_mean", "mean"),
            mean_pred_n_clusters=("pred_n_clusters_mean", "mean"),
            mean_pred_cluster_size_cv=("pred_cluster_size_cv_mean", "mean"),
            mean_known_label_ratio=("known_label_ratio_mean", "mean"),
    ...
    # Best condition per dataset by NMI.
    best = (
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Comprehensive branch-structure comparison on urban real datasets.")
    parser.add_argument("--datasets", type=str, default=",".join(DEFAULT_URBAN_DATASETS))
    ...
    parser.add_argument("--train_log_interval", type=int, default=5)
    ...
    parser.add_argument("--known_only_eval", action="store_true",
                        help="Remap unknown labels to -1 before evaluation (known-only metrics).")
    parser.add_argument("--tag", type=str, default="urban_branch_compare_v1")
    ...
    if cond_filter:
        ...
        conditions = [c for c in conditions if str(c["condition"]) in selected]
        if not conditions:
        ...
        for cond in conditions:
            ...
            for s in seeds:
                ...
                if metrics_path.exists() and not args.force_rerun:
                    ...
                    ari = np.nan
                    acc = np.nan
                    nmi_trial_std = np.nan
                    ari_trial_std = np.nan
                    stability_pair_nmi = np.nan
                    si_loss = np.nan
                    modularity = np.nan
                    conductance_mean = np.nan
                    conductance_weighted = np.nan
                    pred_n_clusters = np.nan
                    pred_cluster_size_cv = np.nan
                    known_label_ratio = np.nan
                    try:
                        m = json.loads(metrics_path.read_text(encoding="utf-8"))
                        nmi = safe_float(m.get("nmi_mean"))
                        ari = safe_float(m.get("ari_mean"))
                        acc = safe_float(m.get("acc_mean"))
                        nmi_trial_std = safe_float(m.get("nmi_trial_std_mean"))
                        ari_trial_std = safe_float(m.get("ari_trial_std_mean"))
                        stability_pair_nmi = safe_float(m.get("stability_pair_nmi_mean"))
                        si_loss = safe_float(m.get("si_loss_mean"))
                        modularity = safe_float(m.get("modularity_mean"))
                        conductance_mean = safe_float(m.get("conductance_mean"))
                        conductance_weighted = safe_float(m.get("conductance_weighted_mean"))
                        pred_n_clusters = safe_float(m.get("pred_n_clusters_mean"))
                        pred_cluster_size_cv = safe_float(m.get("pred_cluster_size_cv_mean"))
                        known_label_ratio = safe_float(m.get("known_label_ratio"))
                    except Exception:
                        pass
                    row = {
                        "dataset": d,
                        ...
                        "nmi": nmi,
                        "ari": ari,
                        "nmi_trial_std": nmi_trial_std,
                        "ari_trial_std": ari_trial_std,
                        "stability_pair_nmi": stability_pair_nmi,
                        "si_loss": si_loss,
                        "modularity": modularity,
                        "conductance_mean": conductance_mean,
                        "conductance_weighted": conductance_weighted,
                        "pred_n_clusters": pred_n_clusters,
                        "pred_cluster_size_cv": pred_cluster_size_cv,
                    ...
                    run_rows.append(row)
                    print(f"[{idx}/{total}] {d} {c_name} s{s} -> {status}")
                    continue

                cmd = build_cmd(repo=repo, dataset=d, seed=s, version=version, cond=cond, args=args)
                t0 = time.time()
                proc = subprocess.run(
                    cmd,
                    cwd=str(repo),
                ...
                run_rows.append(row)
                print(
                    f"[{idx}/{total}] {d} {c_name} s{s} -> {status} "
    ...
    aggregate(out_dir=out_dir, runs=runs)

    summary = {
        "tag": args.tag,
        "datasets": datasets,
        "seeds": seeds,
        "known_only_eval": bool(args.known_only_eval),
        "n_conditions": len(conditions),
        "total_runs": int(len(run_rows)),
        "ok_runs": int((runs["status"] == "ok").sum()),
        "skip_exists_runs": int((runs["status"] == "skip_exists").sum()),
        "fail_runs": int((~runs["status"].isin(["ok", "skip_exists"])).sum()),
