...
class DSI(nn.Module):
    def __init__(self, in_dim, hid_dim, num_nodes, max_nums, temperature=0.2,
                 ...
                 assign_gumbel=True, paper_graph_fusion=False):
        super(DSI, self).__init__()
        self.num_nodes = num_nodes
        self.height = len(max_nums) + 1
        self.manifold = Lorentz()
        self.encoder = LSENet(
            self.manifold, in_dim, hid_dim, max_nums, temperature, dropout, nonlin_str,
            leaf_use_att=leaf_use_att,
            leaf_att_mode=leaf_att_mode,
            num_input_lconvs=num_input_lconvs,
        ...
        self.alpha = alpha
        self.knn = knn
        self.paper_graph_fusion = paper_graph_fusion
    ...
    def se_loss(self, data, eps=1e-6):
        ...
        if self.paper_graph_fusion:
            mixed_adj = (1 - self.alpha) * data.adj + self.alpha * adj_aug
        else:
            mixed_adj = self.alpha * adj_aug + data.adj
        tree_coord_aug_dict, ass_aug_dict, adj_aug_dict = self.encoder(data.x, mixed_adj)
