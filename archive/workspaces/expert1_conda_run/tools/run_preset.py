...
PRESET_MAP = {
    ...
    "b50_v40_assign_only": "configs/presets/b50_v40_assign_only.json",
    "b50_v40_hier_only": "configs/presets/b50_v40_hier_only.json",
    "ep1_main": "configs/presets/v40a_edge_state_main.json",
    "ep1_context_off": "configs/presets/v40a_context_off.json",
    "ep1_support_only": "configs/presets/v40a_support_only.json",
    "ep1_no_hier": "configs/presets/v40a_no_hier.json",
    "v40a_edge_state_main": "configs/presets/v40a_edge_state_main.json",
    "v40a_context_off": "configs/presets/v40a_context_off.json",
    "v40a_support_only": "configs/presets/v40a_support_only.json",
    "v40a_no_hier": "configs/presets/v40a_no_hier.json",
...
def main() -> None:
    ...
    cmd = [
        ...
        "--edge_state_hidden_dim",
        str(preset.get("edge_state_hidden_dim", 128)),
        "--edge_state_temp",
        str(preset.get("edge_state_temp", 1.0)),
        "--edge_state_support_floor",
        str(preset.get("edge_state_support_floor", 0.15)),
        "--edge_state_support_lambda",
        str(preset.get("edge_state_support_lambda", 1.0)),
        "--edge_state_boundary_lambda",
        str(preset.get("edge_state_boundary_lambda", 1.0)),
        "--edge_state_conf_lambda",
        str(preset.get("edge_state_conf_lambda", 5e-3)),
        "--edge_state_balance_lambda",
        str(preset.get("edge_state_balance_lambda", 1e-3)),
        "--edge_attr_pool_topk",
    ...
    if "edge_relation_use_assignment" in preset:
        cmd.append("--edge_relation_use_assignment" if bool(preset.get("edge_relation_use_assignment", True)) else "--no_edge_relation_use_assignment")
    if "edge_relation_use_si_hierarchy" in preset:
        cmd.append("--edge_relation_use_si_hierarchy" if bool(preset.get("edge_relation_use_si_hierarchy", True)) else "--no_edge_relation_use_si_hierarchy")
    if "edge_relation_use_msg_adj" in preset:
        cmd.append("--edge_relation_use_msg_adj" if bool(preset.get("edge_relation_use_msg_adj", True)) else "--no_edge_relation_use_msg_adj")
    if bool(preset.get("edge_state_detach_context", False)):
        cmd.append("--edge_state_detach_context")
    if "edge_state_use_context" in preset:
        cmd.append("--edge_state_use_context" if bool(preset.get("edge_state_use_context", True)) else "--no_edge_state_use_context")
