...
class Map(ipyleaflet.Map):
    ...
    def check_osm_buildings(self,
        ...
        try:
            extract = find_smallest_extract(
                aoi_geometry=self.polygon_bounds.geometry[0],
                data_dir=self.directory,
            )
            filename = f"{extract['source']}_{os.path.basename(extract['url'])}"
            fp = os.path.join(self.directory, filename)
            download_extract(extract["url"], fp, expected_size=extract.get("size"))
            print('Data extracted successfully. Checking OSM building attributes.')
        except ValueError:
        ...
        if self.network:
            ...
            if network_filepath == '':
                try:
                    ...
                    filename = f"{extract['source']}_{os.path.basename(extract['url'])}"
                    network_filepath = os.path.join(self.directory, filename)
                    download_extract(extract["url"], network_filepath, expected_size=extract.get("size"))
                except ValueError:
                    raise ValueError('No osm data found for specified location.')
    ...
    def get_urban_graph(
        ...
        # If precomputed available, use precomputed
        start = time.time()
        if self.polygon_bounds is None:
            raise Exception('Please delimit a bounding box to construct the urban graph.')
        
        # Get country from polygon bounds
        self.country = get_country_from_polygon(self.polygon_bounds.geometry[0]) 
        print(self.country)
        ...
        if self.network:
            ...
            if network_filepath == '':
                try:
                    ...
                    # Use source + name for the filename to avoid collisions
                    filename = f"{extract['source']}_{os.path.basename(extract['url'])}"
                    network_filepath = os.path.join(self.directory, filename)
                    download_extract(extract["url"], network_filepath, expected_size=extract.get("size"))
