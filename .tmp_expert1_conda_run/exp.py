    ...
    def train(self):
        ...
        for exp_iter in range(self.configs.exp_iters):
            ...
            model = DSI(
                ...
                edge_state_hidden_dim=int(getattr(self.configs, "edge_state_hidden_dim", 128)),
                edge_state_temp=float(getattr(self.configs, "edge_state_temp", 1.0)),
                edge_state_support_floor=float(getattr(self.configs, "edge_state_support_floor", 0.15)),
                edge_state_support_lambda=float(getattr(self.configs, "edge_state_support_lambda", 1.0)),
                edge_state_boundary_lambda=float(getattr(self.configs, "edge_state_boundary_lambda", 1.0)),
                edge_state_conf_lambda=float(getattr(self.configs, "edge_state_conf_lambda", 5e-3)),
                edge_state_balance_lambda=float(getattr(self.configs, "edge_state_balance_lambda", 1e-3)),
                edge_state_detach_context=bool(getattr(self.configs, "edge_state_detach_context", False)),
                edge_state_use_context=bool(getattr(self.configs, "edge_state_use_context", True)),
                edge_relation_use_assignment=bool(getattr(self.configs, "edge_relation_use_assignment", True)),
                edge_relation_use_si_hierarchy=bool(getattr(self.configs, "edge_relation_use_si_hierarchy", True)),
                edge_relation_use_msg_adj=bool(getattr(self.configs, "edge_relation_use_msg_adj", True)),
        ...
        edge_state_support_same_stats = self._mean_std([s.get("final_edge_state_support_same_mean", float("nan")) for s in run_stats])
        edge_state_boundary_same_stats = self._mean_std([s.get("final_edge_state_boundary_same_mean", float("nan")) for s in run_stats])
        ...
        return {
            ...
            "final_edge_state_support_same_mean": edge_state_support_same_stats["mean"],
            "final_edge_state_support_same_std": edge_state_support_same_stats["std"],
            "final_edge_state_boundary_same_mean": edge_state_boundary_same_stats["mean"],
            "final_edge_state_boundary_same_std": edge_state_boundary_same_stats["std"],
    ...
    def train_clu(self, data, model, optimizer, logger):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch == 1 or epoch == epochs or epoch % train_log_interval == 0:
                ...
                state_s = adaptive_stats.get("edge_state_support_mean", float("nan"))
                state_b = adaptive_stats.get("edge_state_boundary_mean", float("nan"))
                state_n = adaptive_stats.get("edge_state_neutral_mean", float("nan"))
                state_same_s = adaptive_stats.get("edge_state_support_same_mean", float("nan"))
                state_same_b = adaptive_stats.get("edge_state_boundary_same_mean", float("nan"))
                logger.info(
                    ...
                    f"state[s/b/n]={state_s:.4f}/{state_b:.4f}/{state_n:.4f}, "
                    f"same[s/b]={state_same_s:.4f}/{state_same_b:.4f}"
                )
        ...
        return {
            ...
            "final_edge_state_support_mean": float(edge_stats.get("edge_state_support_mean", 0.0)),
            "final_edge_state_boundary_mean": float(edge_stats.get("edge_state_boundary_mean", 0.0)),
            "final_edge_state_neutral_mean": float(edge_stats.get("edge_state_neutral_mean", 0.0)),
            "final_edge_state_entropy_mean": float(edge_stats.get("edge_state_entropy_mean", 0.0)),
