...
class EdgeStatePredictor(nn.Module):
    """Predict edge-state simplex q=[support, boundary, neutral] on sparse edges."""
    ...
    def __init__(
        ...
        super().__init__()
        self.manifold = manifold
        self.num_roles = int(max(2, num_roles))
        self.temp = float(max(1e-3, temp))
        self.use_context = bool(use_context)
        eucl_dim = int(max(1, node_dim - 1))
        attr_dim = int(max(1, raw_edge_attr_dim))
        in_dim = 4 * eucl_dim + attr_dim + 4
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, self.num_roles),
        )
    ...
    def forward(self, z_leaf, target_adj, edge_attr_aligned=None, matched=None):
        ...
        ui = u[src]
        uj = u[dst]
        lor_score = (2 + 2 * self.manifold.inner(z_leaf[src], z_leaf[dst], keepdim=False)).unsqueeze(1).float()
        ...
        if not self.use_context:
            ...
            lor_score = torch.zeros_like(lor_score)
            lor_dist = torch.zeros_like(lor_dist)
        ...
        if edge_attr_aligned is None:
            edge_attr_aligned = torch.zeros((src.shape[0], 1), dtype=ui.dtype, device=ui.device)
        elif edge_attr_aligned.dim() == 1:
            edge_attr_aligned = edge_attr_aligned.unsqueeze(1)
        elif edge_attr_aligned.dim() > 2:
            edge_attr_aligned = edge_attr_aligned.reshape(edge_attr_aligned.shape[0], -1)
        feat.append(edge_attr_aligned.float().to(device=ui.device, dtype=ui.dtype))

        if matched is None:
            matched_f = torch.ones((src.shape[0], 1), dtype=ui.dtype, device=ui.device)
        else:
            matched_f = matched.float().unsqueeze(1).to(device=ui.device, dtype=ui.dtype)
        feat.append(matched_f)

        logits = self.mlp(torch.cat(feat, dim=1)) / self.temp
    ...
    def forward(self, x, adj, edge_attr=None, use_edge_attr=False):
        ...
        if self.edge_variant == 'V40' and bool(use_edge_attr) and edge_attr is not None:
            ...
            if edge_attr.shape[0] == edge_value.shape[0] and edge_attr.shape[1] >= 3:
                ...
                graph_alpha = self._graph_alpha(confidence, fallback_dtype=score.dtype, fallback_device=score.device)
