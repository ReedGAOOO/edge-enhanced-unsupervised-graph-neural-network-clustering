...
from modules.layers import EdgeStatePredictor, LorentzBoost
...
class DSI(nn.Module):
    def __init__(self, in_dim, hid_dim, num_nodes, max_nums, temperature=0.2,
        ...
        encoder_edge_attr_dim = self.edge_state_num_roles if str(edge_variant).upper() == 'V40' else self.raw_edge_attr_dim
        self.encoder = LSENet(
            self.manifold,
            in_dim,
            hid_dim,
        ...
        self.edge_state_hidden_dim = int(max(16, edge_state_hidden_dim))
        self.edge_state_temp = float(max(1e-3, edge_state_temp))
        self.edge_state_support_floor = float(min(0.95, max(0.0, edge_state_support_floor)))
        self.edge_state_support_lambda = float(max(0.0, edge_state_support_lambda))
        self.edge_state_boundary_lambda = float(max(0.0, edge_state_boundary_lambda))
        self.edge_state_conf_lambda = float(max(0.0, edge_state_conf_lambda))
        self.edge_state_balance_lambda = float(max(0.0, edge_state_balance_lambda))
        self.edge_state_detach_context = bool(edge_state_detach_context)
        ...
        if self._use_edge_state_variant():
            self.edge_state_predictor = EdgeStatePredictor(
                ...
                hidden_dim=self.edge_state_hidden_dim,
                num_roles=self.edge_state_num_roles,
                dropout=dropout,
                temp=self.edge_state_temp,
                use_context=self.edge_state_use_context,
            )
        else:
    ...
    def _build_edge_state_context(self, x, adj_base_msg):
    ...
    def _predict_edge_state_for_adj(self, z_ctx, base_adj, base_edge_attr, target_adj):
        if (not self._use_edge_state_variant()) or self.edge_state_predictor is None:
            return None, None, None, None
        raw_attr, matched = self._align_edge_attr_to_adj_with_mask(base_adj, base_edge_attr, target_adj)
        logits, q = self.edge_state_predictor(
            z_ctx,
            target_adj,
            edge_attr_aligned=raw_attr,
            matched=matched,
        )
        return logits, q, raw_attr, matched

    def _edge_state_factor(self, q_state: torch.Tensor | None) -> torch.Tensor | None:
        if q_state is None or q_state.numel() == 0:
            return None
        q_s = q_state[:, 0].clamp(0.0, 1.0)
        floor = float(self.edge_state_support_floor)
        return floor + (1.0 - floor) * q_s

    def forward(self, data):
        features = data.x
        adj_base_msg = getattr(data, "adj_msg", data.adj)
        adj = adj_base_msg.clone()
        if self._use_learnable_edge_weight_variant():
        ...
        edge_mask = None
        if use_edge_attr and self._use_edge_state_variant():
            z_ctx = self._build_edge_state_context(features, adj_base_msg)
            _, edge_attr, _, _ = self._predict_edge_state_for_adj(z_ctx, adj_base_msg, edge_attr, adj)
            adj = self._apply_state_factor_to_adj(adj, self._edge_state_factor(edge_attr), normalize_for_message=True, head='msg')
        elif use_edge_attr and self._use_edge_relation_variant():
            edge_attr, edge_mask = self._encode_relation_state_for_adj(adj_base_msg, edge_attr, adj)
    ...
    def se_loss(self, data, eps=1e-6):
        ...
        if self._use_edge_state_variant():
            z_ctx = self._build_edge_state_context(data.x, adj_base_msg)
            if self._use_edge_knn_mode(data.x.shape[0]):
                with torch.no_grad():
                    adj_aug = self._edge_candidate_adj(z_ctx, adj_base_msg, self.knn, edge_bias=aug_prior_bias)
            else:
            ...
            _, q_msg, _, _ = self._predict_edge_state_for_adj(z_ctx, adj_base_msg, base_edge_attr, adj_train_msg_raw)
            _, q_si, _, _ = self._predict_edge_state_for_adj(z_ctx, adj_base_si, base_edge_attr, adj_train_si_raw)

            adj_train_msg = self._apply_state_factor_to_adj(
                adj_train_msg_raw,
                self._edge_state_factor(q_msg),
                normalize_for_message=True,
                head='msg',
            )
            adj_train_si = self._apply_state_factor_to_adj(
                adj_train_si_raw,
                self._edge_state_factor(q_si),
            ...
            loss = loss + self._edge_state_aux_loss(ass_aug_dict, adj_train_msg_raw, q_msg)
            return loss
    ...
    def _apply_state_factor_to_adj(self, target_adj: torch.Tensor, factor: torch.Tensor | None, normalize_for_message: bool, head: str = 'msg') -> torch.Tensor:
        tgt = target_adj.coalesce() if target_adj.is_sparse else target_adj.to_sparse().coalesce()
        if factor is None or factor.numel() == 0:
            return self._normalize_sparse_by_degree(tgt) if normalize_for_message else tgt
        factor = factor.to(dtype=tgt.values().dtype, device=tgt.values().device).clamp_min(1e-4)
        self._update_relation_factor_stats(factor, head=head)
    ...
    def _edge_state_aux_loss(self, ass_aug_dict, adj_leaf_raw, q_leaf):
        if (not self._use_edge_state_variant()) or q_leaf is None or q_leaf.numel() == 0:
            return adj_leaf_raw.values().new_zeros(())

        adj = adj_leaf_raw.coalesce() if adj_leaf_raw.is_sparse else adj_leaf_raw.to_sparse().coalesce()
        idx = adj.indices()
        ...
        self.last_edge_state_support_same_mean = float(((w * q_s * same).sum() / sup_denom).detach().item())
        self.last_edge_state_boundary_same_mean = float(((w * q_b * same).sum() / bnd_denom).detach().item())
