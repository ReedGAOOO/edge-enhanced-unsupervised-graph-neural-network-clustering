#!/usr/bin/env python3
"""
Batch-build urban plot graph V3 series and aggregate metadata summaries.
"""
...
DEFAULT_VARIANTS = ["v3s", "v3sj", "v3sjg"]
...
def _write_summary(summary_dir: Path, rows: list[dict]) -> None:
    ...
    df.to_csv(summary_dir / "urban_plot_v3_summary.csv", index=False)
    (summary_dir / "urban_plot_v3_summary.json").write_text(
    ...
    readme = "\n".join(
        [
            ...
            "",
            "Files:",
            "- `urban_plot_v3_summary.csv`: flat manifest across all generated city/variant datasets.",
            "- `urban_plot_v3_summary.json`: JSON version of the same manifest.",
            "",
            "Variant semantics:",
            "- `v3s`: shared-street topology only.",
            "- `v3sj`: shared-street + shared-junction topology with relation-aware pruning.",
            "- `v3sjg`: `v3sj` plus geometric fallback edges only for structurally under-connected plots.",
            "",
            "Design note:",
            "- Feature similarity is not allowed to create edges in V3; it is retained only as descriptive node context.",
        ]
    )
...
def main() -> None:
    ...
    builder = (repo_root / "tools" / "prepare_urban_plot_graph_v3.py").resolve()
    ...
    print(f"[ok] generated {len(rows)} V3 datasets")
